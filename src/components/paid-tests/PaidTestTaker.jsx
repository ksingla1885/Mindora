...
import { PaidTestSecurityOverlay } from './PaidTestSecurityOverlay';
import { PaidTestSecurityBar } from './PaidTestSecurityBar';
...
export function PaidTestTaker({ test, questions: initialQuestions = [], onComplete, initialAttempt, apiBaseUrl = '/api/test-attempts' }) {
  ...
  const handleLogViolation = useCallback(async (violation) => {
    // For paid tests, we only keep light browser restrictions (right-click, copy-paste, keyboard shortcuts).
    // These actions are blocked automatically by the hook and a toast is shown.
    // We do NOT want to log these as violations or show the scary warning modal or disqualify the user.
    return;
  }, []);

  const enableFaceDetection = false;
  const enableTabMonitoring = false;
  const enforceFullscreen = false;
  ...
  const handleStartTestClick = useCallback(async () => {
    // Request fullscreen on start to provide a distraction-free experience,
    // but since enforceFullscreen is false, they won't be blocked if they exit.
    try {
      ...
      console.error('Failed to enter fullscreen on test start:', err);
    }
      ...
      <div className="flex items-center justify-center min-h-[80vh] p-4">
        <div className="w-full max-w-2xl">
          ...
          <div className="text-center mb-6">
            ...
            <h1 className="text-3xl font-black tracking-tight">Test Rules &amp; Briefing</h1>
            <p className="text-muted-foreground mt-1">
              Please read and acknowledge all rules before starting
            </p>
          </div>
          ...
          <div className="space-y-3 mb-6">
            <p className="text-sm font-bold text-muted-foreground uppercase tracking-wide">Active Rules</p>
          ...
          >
            ...
            <span className="text-sm leading-relaxed">
              I have read and understand all test instructions and rules. I agree to abide by them during the test.
            </span>
          </label>
  ...
  return (
    <div className={cn("space-y-6 select-none")} ref={mainContentRef}>
      {/* Security bar — fixed at top during active test */}
      <PaidTestSecurityBar
        isFullscreen={isFullscreen}
        isTabMonitoring={enableTabMonitoring}
        violationCount={warningCount}
        isProctoringActive={isProctoringActive && (enableTabMonitoring || enforceFullscreen)}
        isVisible={hasStarted && !isSubmitting}
      />

      {/* Fullscreen-exit blocking overlay */}
      <PaidTestSecurityOverlay
        isVisible={hasStarted && isProctoringActive && enforceFullscreen && !isFullscreen && !isSubmitting}
